...
describe('Auth routes', () => {
  describe('POST /auth/signup', () => {
    ...
    test('should return 201 and successfully register user if request data is ok', async () => {
      const res = await request(app).post('/auth/signup').send(newUser).expect(httpStatus.CREATED);
    ...
    test('should return 400 error if email is invalid', async () => {
      ...
      await request(app).post('/auth/signup').send(newUser).expect(httpStatus.BAD_REQUEST);
    });
  ...
  describe('POST /auth/signin', () => {
    test('should return 200 and login user if email and password match', async () => {
      ...
      const res = await request(app).post('/auth/signin').send(loginCredentials).expect(httpStatus.OK);
    ...
    test('should return 401 error if there are no users with that email', async () => {
      ...
      const res = await request(app).post('/auth/signin').send(loginCredentials).expect(httpStatus.UNAUTHORIZED);
